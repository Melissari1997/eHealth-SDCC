...
def selectSensors(actualZone):
    ...
    if len(sensors) == 1:
        select = randint(0, len(SensorList) - 2)
        sensors.append(SensorList[select])
    return sensors
        ...
        for surname in SurnameList:
            for city in BirthPlace:
                for birthDay in BirthDayList:
                    actualZone = randint(0, len(fogList) - 1)
                    sensors = selectSensors(actualZone)
...
def createSmallDataset():
    ...
    with open("settings/small.json", "w") as smallDataset:
        smallList = []
        for i in range(0, 10):
...
def createMediumDataset():
    ...
    with open("settings/medium.json", "w") as mediumDataset:
        mediumList = []
...
def createHighDataset():
    ...
    with open("settings/big.json", "w") as bigDataset:
        bigList = []
